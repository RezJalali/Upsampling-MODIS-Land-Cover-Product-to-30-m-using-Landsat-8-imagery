...
// import Landsat 8 images with 30m resolution
...
var l8_preprocess = function (image) {
    ...
    var processed = image.clip(geometry);
    var bands = processed.select('SR_B[2-7]');
    var tir = processed.select('ST_B10');
    
    // Note: It's better to scale after calculating indices with the original values
...
print(seasonal_images,'seasonal_images');
...
var full_sample = mixed.stratifiedSample({
    numPoints: 200, // Increase points for better training/validation
    classBand: 'LC_Type2',
    region: geometry,
    scale: 30,
    geometries: true
...
print(train_acc, 'Training Confusion Matrix (Seasonal)');
print(train_acc.accuracy(), 'Train Accuracy (Seasonal)');

// Validation Accuracy
var validated = validation_data.classify(classifier);
var test_accuracy = validated.errorMatrix('LC_Type2', 'classification');
print(test_accuracy, 'Validation Confusion Matrix (Seasonal)');
print(test_accuracy.accuracy(), 'Validation Accuracy (Seasonal)');
print(test_accuracy.kappa(), 'Validation Kappa (Seasonal)');
